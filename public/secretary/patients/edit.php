...
if (($_SERVER['REQUEST_METHOD'] == 'GET')) {
    ...
    $stmt = $dbh->prepare('UPDATE Patient SET FirstName=?, LastName=?, DateOfBirth=?, Gender=?, ContactNumber=?, EmailAddress=?, Address=? WHERE ID=?');
    $stmt->execute([$_POST['first_name'], $_POST['last_name'], $_POST['date_of_birth'], $_POST['gender'], $_POST['contact_number'], $_POST['email_address'], $_POST['address'], $_POST['insurance_number']]);
...
<body>
    ...
    <form class="max-w-md mx-auto" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <div class="my-4 flex items-center gap-2">
            ...
            <input name="insurance_number" value="<?php echo $result['ID']; ?>" id="insurance_number" type="text" class="bg-gray-200 text-gray-700 flex-1 rounded-md p-2" disabled>
        </div>
        <div class="my-4 flex items-center gap-2">
