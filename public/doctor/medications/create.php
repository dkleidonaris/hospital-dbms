...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['insurance_id'])) {
        $stmt = $dbh->prepare('SELECT FirstName, LastName from Patient WHERE ID=?');
        $stmt->execute(array($_GET['insurance_id']));

        $result = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($stmt->rowCount() < 1) {
            die("No patient with the given insurance Number!");
        }
    } else {
        die('That is not allowed!');
    }
} elseif ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $stmt = $dbh->prepare('INSERT INTO Medication (PatientID, DoctorID, Name, Dosage, Frequency, StartDate, EndDate, OtherDescription) VALUES(?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([$_POST['patient_id'], $_POST['doctor_id'], $_POST['med_name'], $_POST['dosage'], $_POST['frequency'], $_POST['start_date'], $_POST['end_date'], $_POST['description']]);

    $_SESSION['message_type'] = 'info';
    $_SESSION['message'] = 'The medication has been created!';
...
<body>
    ...
    <form class="max-w-md mx-auto" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        <input name="doctor_id" value="<?php echo $_SESSION['employee_id']; ?>" type="text" class="hidden">
        <div class="my-4 flex items-center gap-2">
            <label for="patient" class="min-w-36">Patient:</label>
            <input name="patient_id" id="patient" value="<?php echo $_GET['insurance_id']; ?>" type="text" class="w-full  text-gray-700 bg-gray-400 rounded-md p-2 flex-1" readonly>
        </div>
        <div class="my-4 flex items-center gap-2">
            <label for="patient_name" class="min-w-36">Patient Name:</label>
            <input id="patient_name" value="<?php echo $result['LastName'] . " " . $result['FirstName']; ?>" type="text" class="w-full  text-gray-700 bg-gray-400 rounded-md p-2 flex-1" readonly>
        </div>
        <div class="my-4 flex items-center gap-2">
            <label for="med_name" class="min-w-36">Medication Name:</label>
            <input name="med_name" id="med_name" type="text" class="flex-1 rounded-md p-2">
        </div>
        <div class="my-4 flex items-center gap-2">
            <label for="dosage" class="min-w-36">Dosage</label>
            <input name="dosage" id="dosage" type="text" class="flex-1 rounded-md p-2">
        </div>
        <div class="my-4 flex items-center gap-2">
            <label for="frequency" class="min-w-36">Frequency:</label>
            <input name="frequency" id="frequency" type="text" class="flex-1 rounded-md p-2">
        </div>
        <div class="my-4 flex items-center gap-2">
            <label for="start_date" class="min-w-36">Start Date:</label>
            <input name="start_date" id="start_date" type="date" class="flex-1 rounded-md p-2" required>
        </div>
        <div class="my-4 flex items-center gap-2">
        ...
        <div class="my-4 flex items-center gap-2">
            ...
            <textarea id="description" name="description" rows="4" cols="50" class="p-2 rounded-md"></textarea>
        </div>
