...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($DELETE['id'])) {
        $stmt = $dbh->prepare('SELECT * FROM Admission WHERE ID=:id');
        $stmt->execute([':id' => $DELETE['id']]);
        if ($stmt->rowCount() > 0) {
            $dbh->prepare('DELETE FROM Admission WHERE id=?')->execute([$DELETE['id']]);

            $response['status'] = 'The admission has been deleted';
            echo json_encode($response);
            exit;
        } else {
            $response['status'] = 'The admission that you are trying to delete does not exist!';
            echo json_encode($response);
        ...
        $response['status'] = 'Please provide admission id for deletion!';
        echo json_encode($response);
